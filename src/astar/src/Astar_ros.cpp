...
//OVERRIDE THIS FUNC- ADD IIG HERE
...
void AstarPlannerROS::getCoordinate(float& x, float& y)
{
...
for (uint i=0; i<mapSize; i++)
  ...
  bestPath = findPath(startCell, goalCell,  g_score);
   ...
   cout<<"time to generate best global path by A* = " << (diff(time1,time2).tv_sec)*1e3 + (diff(time1,time2).tv_nsec)*1e-6 << " microseconds" << endl;
...
/*********************************************************************************/
vector<int> AstarPlannerROS::constructPath(int startCell, int goalCell,float g_score[])
{
	vector<int> bestPath;
	vector<int> path;
...
/*********************************************************************************/
/*
float AstarPlannerROS::calculateHCost(int cellID, int goalCell)
{
...
/*********************************************************************************/
void AstarPlannerROS::addNeighborCellToOpenList(multiset<cells> & OPL, int neighborCell, int goalCell, float g_score[])
{
	cells CP;
